...
export const ContentCreationRequest = async (req: Request, res:Response) => {
    try {
        ...
        if(!_contentReq) {
            ...
            return getResponseMessage(req, res, 400, `Content Creation failed  | ${_contentReq}`)
        }
        return getResponseMessage(req, res, 201, _contentReq)
    } catch (error) {
        logger.error(`Unable to create Content for ${req?.user?.id} | details=${getErrorMessage(error)}`)
        return getResponseMessage(req, res, 500, `Content failed while creating ${getErrorMessage(error)}`)
    }
}
...
export const GetContentRequest = async (req:Request, res:Response) => {
    try {
        ...
        if(!_getContent) {
            ...
            return getResponseMessage(req, res, 400, `Content fetching failed  | ${_getContent}`)
        }
        ...
        logger.error(`Unable to fetch Content for ${req?.user?.id} | details=${getErrorMessage(error)}`)
        return getResponseMessage(req, res, 500, `Content failed while fetching ${getErrorMessage(error)}`)
    }
}
...
export const DeleteContentRequestbyId = async (req:Request, res:Response) => {
    try {
        ...
        logger.error(`Unable to delete Content for ${req?.user?.id} | details=${getErrorMessage(error)}`)
        return getResponseMessage(req, res, 500, `Content failed while deleting ${getErrorMessage(error)}`)
    }
}


export const DeleteContentRequestbyAll = async (req:Request, res:Response) => {
    try {
        const userId= String(req?.user?.id)
...
export const UpdateContentRequestbyId = async (req:Request, res:Response) => {
    try {
        ...
        const userId = String(req?.user?.id)
        const _updateContent = await contentRepo.updateContentbyId(userId,contentId,updatedContent)
        if(!_updateContent?.id) {return getResponseMessage(req, res, 400, "Invalid Id")}
        return getResponseMessage(req, res, 200, _updateContent)
    }catch (error) {
        logger.error(`Unable to update Content for ${req?.user?.id} | details=${getErrorMessage(error)}`)
        return getResponseMessage(req, res, 500, `Content failed while updating ${getErrorMessage(error)}`)
    }
}
