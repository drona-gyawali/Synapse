...
export const shareRequest = async (req:Request, res:Response) => {
    try {
        ...
        if(_share === "true") {
            ...
            logger.info(`Link created  for ${req?.user?.id}| details:${linkCreated?.userId}`)
        }else {
            const delLink = await link.deleteLink(String(req?.user?.id)) 
            logger.info(`Link deleted for ${req?.user?.id} | status=${delLink}`)
        }
        return getResponseMessage(req, res, 200,  _share == "false" ? "Link removed": linkCreated?.hash )
    } catch (error) {
        logger.error(`Link error while operating share operation for ${String(req?.user?.id)} | error=${getErrorMessage(error)}`)        
    }
}
...
export const receiverLinkRequest = async (req: Request, res: Response) => {
    try {
        ...
        if(!userHash) {return getResponseMessage(req, res, 400, "Invalid link")}
        const content = await contentRepo.getContentPublic(userHash?.userId, Number(skip), Number(limit))
        if(content?.length == 0) {
            return getResponseMessage(req, res, 400, "No user content or user is private")
        }
