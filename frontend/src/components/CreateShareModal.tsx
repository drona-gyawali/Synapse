            ...
            <div className="relative bg-white w-full max-w-md p-6 rounded-2xl shadow-2xl transform transition-all animate-in fade-in zoom-in duration-200">
                <div>
                    <div className="text-gray-500 font-bold flex items-center  gap-3">
                        <div className=""><BrainCogIcon size={24} /></div>
                        ...
                        onClick={props.close}
                    >
                        <div className="ml-[10.9rem] md-2"><X size={20} /></div>
                    </div>
                    </div>
                    <div className="text-xs text-gray-600 font-semibold ml-[9%]">
                        <span>Share you brain with world</span>
                    </div>
                    {!mutation.isSuccess ? <div className="flex  justify-center m-[7%] ">
                        <Button
                    ...
                    {mutation.isSuccess ? <div className="flex  items-center justify-center gap-3">
                        <div className=" break-word truncate "><Input style={"cursor-text border-gray-300 w-[1/2] bg-[#fffbf1]"} type="text" placeholder={FRONTEND_URL + mutation.data?.details} isdisabled={true} /></div>
                        <div className="cursor-pointer text-gray-500 mt-4 text-1xl" onClick={handleCopy}> {!copy ? <Copy size={20} /> : <CopyCheckIcon size={18} />} </div>
                    </div> : ""}
                </div>
            </div>
        </div>
