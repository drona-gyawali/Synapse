...
const defaultStyles = "px-4 py-2 text-center rounded-md font-bold cursor-pointer flex items-center"
        ...
        onClick={props.onClick}
         ...
         `
           ...
           <div className='flex items-center gap-3 '>
            {props.startIcon} 
             <p className={` ${props.style}`} >{!props.loading ? props.text : <Loader2Icon size={18} className='animate-spin'/>}</p>
           </div>
        </button>
        </>
    )
