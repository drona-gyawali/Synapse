...
function Setting(props: SettingOptions) {
  ...
  const handleToggle = (value: any, action: any) => {
        ...
        <div className="relative bg-white w-full max-w-md p-6 rounded-2xl shadow-2xl transform transition-all animate-in fade-in zoom-in duration-200">
          ...
          <div className="space-y-6">
            <form>
              ...
              <div
                className="flex gap-5 mt-3 items-center justify-between"
                title="we collect data for user experience only"
              >
                ...
                <div
                  ...
                  className={`w-14 h-7 flex items-center rounded-full p-1 cursor-pointer transition-colors duration-300 ${
                    !deleteLinks ? 'bg-blue-500' : 'bg-gray-300'
                  }`}
                >
                  <div
                    className={`bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 ${
                      !deleteLinks ? 'translate-x-7' : 'translate-x-0'
                    }`}
                  />
                </div>
              </div>
              ...
              >
                ...
                <div
                  ...
                  className={`w-14 h-7 flex items-center rounded-full p-1 cursor-pointer transition-colors duration-300 ${
                    !grantPersonalData ? 'bg-blue-500' : 'bg-gray-300'
                  }`}
                >
                  <div
                    className={`bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 ${
                      !grantPersonalData ? 'translate-x-7' : 'translate-x-0'
                    }`}
                  />
                </div>
              </div>
