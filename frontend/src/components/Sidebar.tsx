    ...
    return (
        <div className="h-screen bg-white border-r border-gray-200 w-72 fixed left-0 top-0">
            <div className="p-5 ">
                ...
                <div className={ props.currentTab !== "all" ? defaultStyle : proStyle}  onClick={() => props.setSearchParams({tab: "all"})}>
                    <div className="text-gray-400 ">{<HomeIcon size={27}/>}</div>
                    <div  className="font-bold  hover:text-gray-500 text-gray-500 cursor-pointer">
                        Home
                        </div>
                </div>
                <div onClick={() => props.setSearchParams({tab: "x"})} className={ props.currentTab !== "x" ? defaultStyle : proStyle} >
                    <div className="text-gray-400 ">{<TwitterIcon size={27}/>}</div>
                    <div  className="font-bold  hover:text-gray-500 text-gray-500 cursor-pointer">
                        Tweets
                        </div>
                </div>
                <div onClick={() => props.setSearchParams({tab: "youtube"})} className={ props.currentTab !== "youtube" ? defaultStyle : proStyle} >
                    <div className="text-gray-400 ">{<VideoIcon size={27}/>}</div>
                    <div onClick={() => props.setSearchParams({tab: "youtube"})} className="font-bold hover:text-gray-500 text-gray-500 cursor-pointer">
                        Videos
                        </div>
                </div>
